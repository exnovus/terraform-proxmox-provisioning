# Envcommon for LXC stacks. Architecture rule: use only components/lxc-service as the source. docs/en/architecture.md
locals {
  ...
  stack_ct_id_base = lookup(local.stack_vars, "default_ct_id_base", null)
...
# Architecture rule: components only; never resources/proxmox-lxc.
terraform {
  source = "${get_repo_root()}/modules//components/lxc-service"
}
...
inputs = {
  ...
  default_os_template  = lookup(local.proxmox_defaults, "default_ct_template", null)
  default_ct_id_base   = local.stack_ct_id_base != null ? local.stack_ct_id_base : lookup(local.proxmox_defaults, "default_ct_id_base", 200)

  core_tags   = local.core_tags
  custom_tags = local.custom_tags
  tags        = local.final_tags

  ct_groups = lookup(local.stack_vars, "ct_groups", {})
}
